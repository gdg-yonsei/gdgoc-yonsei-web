...
const copy = {
  ...
  title: { en: 'Projects by Generation', ko: '기수별 프로젝트' },
} as const
...
export default function ProjectIndex({ params }: Props) {
  return (
    <GenerationIndexShell copy={copy}>
      <Suspense fallback={<GenerationIndexFallback copy={copy} />}>
        <ProjectIndexContent params={params} />
      </Suspense>
    </GenerationIndexShell>
  )
}

async function ProjectIndexContent({ params }: Props) {
